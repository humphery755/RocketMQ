...
class ProtocolProcessor implements EventHandler<ValueEvent> {
    ...
    static final class WillMessage {
        ...
        public WillMessage(String topic, ByteBuffer payload, boolean retained, byte qos) {
            this.topic = topic;
    ...
    private void processPublish(String clientID, String topic, byte qos, ByteBuffer message, boolean retain, Integer messageID) { 
        LOG.info("Publish recieved from clientID <{}> on topic <{}> with QoS {}", 
                clientID, topic, qos);

        String publishKey = null;
        if (qos == MessageType.QOS_LEAST_ONE) {
        ...
        }  else if (qos == MessageType.QOS_EXACTLY_ONCE) {
            publishKey = String.format("%s%d", clientID, messageID);
     ...
     * */
    private void publish2Subscribers(String topic, byte qos, ByteBuffer origMessage, boolean retain, Integer messageID) {
        ...
        for (final Subscription sub : subscriptions.matches(topic)) {
            ...
            if (qos == MessageType.QOS_MOST_ONE && sub.isActive()) {
                //QoS 0
    ...
    private void sendPublish(String clientId, String topic, byte qos, ByteBuffer message, boolean retained) {
    ...
    private void sendPublish(String clientId, String topic, byte qos, ByteBuffer message, boolean retained, int messageID) {
...
//        m_notifier.disconnect(evt.getSession());
    ...
    void processSubscribe(ServerChannel session, SubscribeMessage msg) {
        ...
        for (SubscribeMessage.Couple req : msg.subscriptions()) {
            byte qos = req.getQos();
            Subscription newSubscription = new Subscription(clientID, req.getTopicFilter(), qos, cleanSession,-1);
    ...
    private void subscribeSingleTopic(Subscription newSubscription, final String topic) {
        LOG.info("<{}> subscribed to topic <{}> with QoS {}", 
                ...
                MessageType.formatQoS(newSubscription.getRequestedQos()));
