...
public class SimpleMessaging implements IMessaging, EventHandler<ValueEvent> {
    ...
    private final ProtocolProcessor2MQ m_processor = new ProtocolProcessor2MQ();
    ...
    public void init(Properties configProps) {
        ...
        ThreadFactory disruptorThreadFactory = new ThreadFactory() {
			...
			public Thread newThread(Runnable r) {
				return new Thread(r, String.format("Messaging.Disruptor.executor%d", this.threadIndex.incrementAndGet()));
			}
		};
        m_executor = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors()+1,disruptorThreadFactory);
        int ringBufferSize = Integer.valueOf(configProps.getProperty("ringBufferSize.io.publish"));//
        m_disruptor = new Disruptor<ValueEvent>(ValueEvent.EVENT_FACTORY, 1024 * ringBufferSize, m_executor);
...
//        disruptorPublish(new InitEvent(configProps));
    ...
    private void processInit(Properties props) {
        ...
        m_processor.init(subscriptions, m_storageService, m_sessionsStore, props);
