...
public class SubAckDecoderTest {
    ...
    public void testBadQos() throws Exception {
        initHeaderQos(m_buff, 0xAABB, MessageType.QOS_LEAST_ONE, MessageType.QOS_MOST_ONE, MessageType.QOS_MOST_ONE);
        ...
        List<Byte> qoses = message.types();
        assertEquals(3, qoses.size());
        assertEquals(MessageType.QOS_LEAST_ONE, (byte)qoses.get(0));
        assertEquals(MessageType.QOS_MOST_ONE, (byte)qoses.get(1));
        assertEquals(MessageType.QOS_MOST_ONE, (byte)qoses.get(2));
        assertEquals(MessageType.SUBACK, message.getMessageType());
    }
    ...
    public void testBugBadRemainingCalculation() throws Exception {
        ...
        assertEquals(MessageType.QOS_LEAST_ONE, (byte)message.types().get(0));
    }

    private void initHeaderQos(ByteBuf buff, int messageID, Byte... qoss) throws IllegalAccessException {
        buff.clear().writeByte(MessageType.SUBACK << 4).
