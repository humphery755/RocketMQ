...
public class MqttClient {
	...
	public void test(){
		...
		do{
			if(isconnect){
				// 订阅主题
				Topic[] topics = { new Topic("G/123", QoS.AT_LEAST_ONCE) };
				...
				// 发布消息
				callbackConnection.publish(topic, ("Hello ").getBytes(), QoS.AT_LEAST_ONCE, true, new Callback<Void>() {
				...
				UTF8Buffer[] utf8buffers = {UTF8Buffer.utf8(topic)};
	...
	public void init() throws Exception{
		MQTT mqtt = new MQTT();
		mqtt.setHost("tcp://192.168.99.12:1883");
		mqtt.setHost("tcp://127.0.0.1:1883");
		mqtt.setClientId("P/clientId-1");
