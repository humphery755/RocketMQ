 ...
 */
package com.alibaba.rocketmq.store.transaction.log;
 ...
 * @since 2013-7-21
 */
public class TransactionLogStore implements TransactionStore {
	// 存储单元大小
	...
	public TransactionLogStore(final DefaultMessageStore messageStore) {
		this.messageStore = messageStore;

		this.tranStateTable = new MapedFileQueue(
				StorePathConfigHelper
						.getTranStateTableStorePath(messageStore.getMessageStoreConfig().getStorePathRootDir()),
				messageStore.getMessageStoreConfig().getTranStateTableMapedFileSize(), null);

		this.tranRedoLog = new ConsumeQueue(//
				TRANSACTION_REDOLOG_TOPIC, //
				TRANSACTION_REDOLOG_TOPIC_QUEUEID, //
				StorePathConfigHelper
						.getTranRedoLogStorePath(messageStore.getMessageStoreConfig().getStorePathRootDir()), //
				messageStore.getMessageStoreConfig().getTranRedoLogMapedFileSize(), //
				messageStore);
	}
	...
	private void addTimerTask(final MapedFile mf) {
		this.timer.scheduleAtFixedRate(new TimerTask() {
			 ...
			 * TransactionLogStore.this.defaultMessageStore.
			 * getTransactionCheckExecuter();
			 */
			private final long checkTransactionMessageAtleastInterval = TransactionLogStore.this.messageStore
					.getMessageStoreConfig().getCheckTransactionMessageAtleastInterval();
			private final boolean slave = TransactionLogStore.this.messageStore.getMessageStoreConfig()
			...
			public void run() {
				...
				// Check功能是否开启
				/*
				 * if (!TransactionLogStore.this.defaultMessageStore.
				 * getMessageStoreConfig() .isCheckTransactionMessageEnable()) {
				 * return; }
				 */
			...
			private long getTranStateOffset(final long currentIndex) {
				long offset = (this.mapedFile.getFileFromOffset() + currentIndex) / TransactionLogStore.TSStoreUnitSize;
				return offset;
			}
			...
			if (bufferConsumeQueue != null) {
				try {
					...
					for (; i < bufferConsumeQueue.getSize(); i += ConsumeQueue.CQStoreUnitSize) {
						...
						// Prepared
						if (TransactionLogStore.PreparedMessageTagsCode == tagsCode) {
	 ...
	 * 单线程调用
	 */
	private boolean appendPreparedTransaction(//
			final long clOffset, //
			final int size, //
			final int timestamp, //
	...
	private SelectMapedBufferResult findTransactionBuffer(final long tsOffset) {
		final int mapedFileSize = this.messageStore.getMessageStoreConfig().getTranStateTableMapedFileSize();
	...
	private boolean updateTransactionState(//
            final long tsOffset,//
            final long clOffset,//
            final int groupHashCode,//
            final int state//
	...
	public boolean execute(DispatchRequest req) {
		...
		// 2、记录Transaction Redo Log
		switch (tranType) {
		case MessageSysFlag.TransactionNotType:
			break;
		case MessageSysFlag.TransactionPreparedType:
			// 记录redolog
			getTranRedoLog().putMessagePostionInfoWrapper(//
					req.getCommitLogOffset(), //
					req.getMsgSize(), //
					PreparedMessageTagsCode, //
